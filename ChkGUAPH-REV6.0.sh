...
#This script uses the API calls from https://http://159.65.221.180:3001/, from https://probit.com and direct calls from the Guapcoin Blockchain
...
do
  ...
  Addr[$n]=$(curl -s http://159.65.221.180:3001/ext/getbalance/$i | awk -F, '{print $3}' | sed 's/.*://')
  ...
  if [[ $tempLabel == *M* ]]; then #Skip addresses that are not labeled as masternodes
    ...
    temp_MNs_dataA="<http://159.65.221.180:3001/address/$i| Masternode $tempLabel>\n "

  else
    #temp_MNs_dataA="<https://guapexplorer.com/#/address/$i| GUAP ID $tempLabel>\n "
    temp_MNs_dataA="<http://159.65.221.180:3001/address/$i| GUAP ID $tempLabel>\n "
  fi
...
parm7=$(curl -s http://159.65.221.180:3001/ext/getmoneysupply | sed 's/^[[:space:]]*//g')
...
parm8=$(guapcoin-cli -conf=/home/guapadmin/.guapcoin1/guapcoin.conf -datadir=/home/guapadmin/.guapcoin1 getmasternodecount | awk '/stable/' | sed 's/.*://' | sed 's/,$//' | sed 's/^[[:space:]]*//g')
...
parm9=$(curl -s http://159.65.221.180:3001/api/getblockcount)
