...
echo -e "${GREEN} ------- Additional GuapCoin MASTERNODE INSTALLER v2.3.2--------+
 |                                                        |
 |                                                        |::
 ...
 |   VPS using the standard GUAP-v2.3.2                   |
...
cd $USERHOME/.guapcoin$MNID/ && wget https://github.com/guapcrypto/Guap-v2.3.2-MN/releases/download/v2.3.2/bootstrap.zip
cd $USERHOME/.guapcoin$MNID/ && unzip bootstrap.zip

rm -rf $USERHOME/.guapcoin$MNID/bootstrap.zip
...
to edit service:      ${GREEN}nano /etc/systemd/system/guapcoin$MNID.service${NC}
