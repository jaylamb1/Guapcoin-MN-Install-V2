...
do
  ...
  var="https://guapexplorer.com/ext/getaddress/$i"
  parm=$(curl -s "$var" | awk -F, '{print $3}' | sed 's/.*://')
  Addr[$n]=$parm
  tempVar=${Addr[$n]}
...
parm7="https://guapexplorer.com/api/coin/"
GUAPTotal=$(curl -s "$parm7" | awk -F, '{print $12}' | sed 's/.*://')
