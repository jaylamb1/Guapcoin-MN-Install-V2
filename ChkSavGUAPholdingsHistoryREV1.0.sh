...
echo "           [GUAP Holdings Historical Snaphot Rev 1.0]            " >> $SnapshotFilename
echo "-----------------------------------------------------------------" >> $SnapshotFilename
...
LastGuapFile="/home/guapadmin/output_history.text"
...
do
  ...
  Addr[$n]=$(curl -s https://www.guapcoinexplorer.com/ext/getaddress/$i | grep -Po '(?<=(\"received\":)).*(?=,\"ba)' | sed 's/.*://' | sed 's/^-//')
  tempVar=${Addr[$n]}
  tempVarTest=false
