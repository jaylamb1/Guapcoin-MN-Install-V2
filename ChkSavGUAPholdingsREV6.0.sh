...
echo "               [GUAP Holdings Snaphot Rev 6.0]                   " >> $SnapshotFilename
echo "-----------------------------------------------------------------" >> $SnapshotFilename
...
do
  ...
  if [[ $tempLabel == *M* ]]; then #Skip addresses that are not labeled as masternodes
    ...
    temp_MNs_dataA="<https://ex1.guapcoin.org/address/$i| Masternode $tempLabel>\n "
  else
    #temp_MNs_dataA="<https://guapexplorer.com/#/address/$i| GUAP ID $tempLabel>\n "
    temp_MNs_dataA="<https://ex1.guapcoin.org/address/$i| GUAP ID $tempLabel>\n "
  fi
...
parm7=$(guapcoin-cli -conf=/home/guapadmin/.guapcoin1/guapcoin.conf -datadir=/home/guapadmin/.guapcoin1 getinfo | awk 'sub(/.*\"moneysupply\":/,""){print $1}' | sed 's/,$//' | sed 's/.....$//')
...
parm8=$(guapcoin-cli -conf=/home/guapadmin/.guapcoin1/guapcoin.conf -datadir=/home/guapadmin/.guapcoin1 getmasternodecount | awk 'sub(/.*\"total\":/,""){print $1}' | sed 's/,$//' | sed 's/ //g' | sed 's/\r$//')
...
parm9=$(guapcoin-cli -conf=/home/guapadmin/.guapcoin1/guapcoin.conf -datadir=/home/guapadmin/.guapcoin1 getblockcount)
