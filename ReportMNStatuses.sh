  ...
  if ! [[ $tempLabel == *M* ]]; then #Skip addresses that are not labeled as masternodes
    ...
    echo "" >> $StatusReportFilename
      ...
      if ! [[ $tempStatus == "ENABLED" ]]; then
        ...
        curl -X POST -H 'Content-type: application/json' --data '{ "blocks": [ { "type": "section", "text": { "type": "plain_text", "text": ":wrench::exclamation:*Alert!* *Status:* '"$tempStatus"' \n", "emoji": true } }, { "type": "divider" }, { "type": "section", "text": { "type": "mrkdwn", "text": "'"$tempLabel"' \n <https://guapexplorer.com/#/address/'"$i"'|'"$i"'> \n '"$d_formatted"' \n\n" } }, { "type": "divider" }, { "type": "section", "block_id": "section567", "text": { "type": "mrkdwn", "text": "Attention Needed: Please take a closer look at '"$tempLabel"'." } } ] }'
